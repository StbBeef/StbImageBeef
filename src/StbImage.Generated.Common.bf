...
namespace StbImageSharp
{
	extension StbImage
	{
		...
		public static void stbi__vertical_flip(void* image, int w, int h, int uint8s_per_pixel)
		{
			...
			{
				var row0 = uint8s + (int64)row * uint8s_per_row;
				var row1 = uint8s + (int64)(h - row - 1) * uint8s_per_row;
				var uint8s_left = uint8s_per_row;
				while (uint8s_left != 0)
				{
					var uint8s_copy = uint8s_left < 2048 ? uint8s_left : 2048;
					CRuntime.memcpy(temp.CArray(), row0, uint8s_copy);
					CRuntime.memcpy(row0, row1, uint8s_copy);
					CRuntime.memcpy(row1, temp.CArray(), uint8s_copy);
					row0 += uint8s_copy;
					row1 += uint8s_copy;
		...
		public static uint16 stbi__get16be(stbi__context s)
		{
			...
			for (j = 0; j < (int)y; ++j)
			{
				...
				switch (img_n * 8 + req_comp)
				{
					...
					case 4 * 8 + 3:
						...
						break;
					default:
						return (uint8*)(stbi__err("0") != 0 ? (uint8*)null : null);
				}
			}
		...
		public static uint16* stbi__convert_format16(uint16* data, int img_n, int req_comp, int x, int y)
		{
			...
			if (req_comp == img_n)
				return data;
			good = (uint16*)stbi__malloc((uint64)(req_comp * x * y * 2));
			if (good == null)
			{
				CRuntime.free(data);
				return (uint16*)(uint8*)(stbi__err("outofmem") != 0 ? (uint8*)null : null);
			...
			for (j = 0; j < (int)y; ++j)
			{
				...
				switch (img_n * 8 + req_comp)
				{
					...
					case 4 * 8 + 3:
						...
						break;
					default:
						return (uint16*)(uint8*)(stbi__err("0") != 0 ? (uint8*)null : null);
				}
			}
