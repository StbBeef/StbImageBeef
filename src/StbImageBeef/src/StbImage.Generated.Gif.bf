		...
		public static uint8* stbi__process_gif_raster(stbi__context s, stbi__gif g)
		{
			...
			for (; ; )
				...
				else
				{
					...
					{
						if (first != 0)
							return (uint8*)(stbi__err("no clear code") != 0 ? (uint8*)null : null);
						if (oldcode >= 0)
						{
							p = g.codes + avail++;
							if (avail > 8192)
								return (uint8*)(stbi__err("too many codes") != 0 ? (uint8*)null : null);
						...
						{
							return (uint8*)(stbi__err("illegal code in raster") != 0 ? (uint8*)null : null);
						}
			...
			for (; ; )
			{
				...
				switch (tag)
				{
					case 0x2C:
					{
						...
						h = stbi__get16le(s);
						if (x + w > g.w || y + h > g.h)
							return (uint8*)(stbi__err("bad Image Descriptor") != 0 ? (uint8*)null : null);
						g.line_size = g.w * 4;
						g.start_x = x * 4;
						...
						{
							return (uint8*)(stbi__err("missing color table") != 0 ? (uint8*)null : null);
						}
					...
					case 0x3B:
						return null;
					default:
						return (uint8*)(stbi__err("unknown code") != 0 ? (uint8*)null : null);
				}
			}
		}
